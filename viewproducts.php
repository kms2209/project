...
<body>
<div style="width:700px; margin:0 auto; position:relative; border:3px solid rgba(0,0,0,0); -webkit-border-radius:5px; -moz-border-radius:5px; border-radius:5px; -webkit-box-shadow:0 0 18px rgba(0,0,0,0.4); -moz-box-shadow:0 0 18px rgba(0,0,0,0.4); box-shadow:0 0 18px rgba(0,0,0,0.4); margin-top:10%;">
...
<div style="float:left; margin-left:10px; margin-top:10px;"><strong>Welcome</strong> <?php echo $_SESSION['SESS_FIRST_NAME'];?></div>
 ...
 <br /><label style="margin-left:12px;">Filter</label> <input type="text" name="filter" value="" id="filter" /> 
  ...
  <a rel="facebox" href="addproduct.php">add product </a> | <a rel="facebox" href="addcategory.php">Add Category </a><br />
   <br />
  <table cellpadding="1" cellspacing="1" id="resultTable" border="1">
         ...
         <?php
			   include('config.php');
								...
								$result3 = mysqli_query($bd,"SELECT * FROM marias_products");
								...
								while($row3 = mysqli_fetch_array($result3))
								 ...
								 echo '<tr>';
									...
									echo '<td>'.$row3['product_ingredients'].'</td>';
									echo '<td>';
